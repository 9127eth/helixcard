...
(async () => {
  ...
  try {
    ...
    // Hold-driven effects own long presses on mobile. Nothing in the card can
    // enter native selection, links included: Safari snaps a long-press on a
    // selectable node that has no text to the nearest selectable text, which
    // highlighted the phone number from far away. Links are still copied through
    // their long-press callout, and an early finger move remains available to
    // native scrolling.
    for (const effect of ['portal', 'portal-grid', 'black-hole', 'glitch', 'scramble', 'repel', 'shatter']) {
      ...
      assert.notEqual(await frame.locator('a').first().evaluate(el => getComputedStyle(el).userSelect), 'text', `${effect} leaves a long press no selectable text to snap to`);
      // The Portal rip puts the held finger over <body>, so that is locked too.
      assert.equal(await frame.locator('body').evaluate(el => getComputedStyle(el).userSelect === 'none'), effect.startsWith('portal'), `${effect} backdrop selection`);
    }
    await update('ripple');
